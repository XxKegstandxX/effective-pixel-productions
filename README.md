...
| Lost the manage link | `/headshots/find` → `POST /api/find-booking` | Email + phone (digits-only compare) → confirmed bookings on the current event. 1 match redirects, several are listed, none gives one generic message. Rate-limited to 5/min per IP via `register_lookup_attempt()` |
